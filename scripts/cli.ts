#!/usr/bin/env ts-node
/**
 * Minimal menu CLI to orchestrate:
 * - scripts/deploy2.ts
 * - scripts/transfer-approve2.ts
 *
 * It streams each child process' stdout/stderr to YOUR console,
 * so the "proof" (tx hash, block number, before/after, events) is visible.
 */

import * as dotenv from "dotenv";
dotenv.config({ path: ".env" }); // change to ".env.local" if you prefer
...
// ------- helpers -------
function runHardhatScript(scriptRelPath: string, extraEnv: Record<string, string> = {}) {
...
async function doTransfer() {
    ...
    // Offer quick entry; fall back to .env if left blank
...
async function doTransferApprove() {
    ...
    // You can override via quick prompt
...
async function doFullDemo() {
    ...
    // Best effort: if TO not set, prompt for a local account
